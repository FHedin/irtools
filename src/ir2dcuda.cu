...
int ir2d_cuda(FILE* input, FILE* output)
{
    ...
    sscanf(buffer,"%f %f %f %f",&w1min,&w1max,&w3min,&w3max);

    fgets(buffer,2048,input);
    removeComments(buffer);
    sscanf(buffer,"%f %f %f",&delta,&tLife,&alpha);

    fgets(buffer,2048,input);
    removeComments(buffer);
    sscanf(buffer,"%d %f",&ndata,&dt);
    ...
    printf("w1min=%f w1max=%f w3min=%f w3max=%f\n",w1min,w1max,w3min,w3max);
    printf("delta=%f tLife=%f alpha=%f\n",delta,tLife,alpha);
    printf("ndata=%d dt=%f\n",ndata,dt);
    printf("t2=%f\n",t2);
    printf("nStructs=%d\n",nave);
    ...
    for(ii=0; ii<nave; ii++)
    {
        ...
        fgets(buffer,2048,input);
        removeComments(buffer);
        sscanf(buffer,"%f",&tau1);

        fgets(buffer,2048,input);
        removeComments(buffer);
        sscanf(buffer,"%f",&tau2);

        fgets(buffer,2048,input);
        removeComments(buffer);
        sscanf(buffer,"%f",&tau3);

        printf("Structure %d : wm=%f a=%f b=%f c=%f tau1=%f tau2=%f tau3=%f\n",ii,wm,c1,c2,c3,tau1,tau2,tau3);
        ...
        cudaMemcpy(d_param,param,13*sizeof(float),cudaMemcpyHostToDevice);
